...
export default function ProfileView({ section }: ProfileViewProps) {
  ...
  const showListings = section === "listings";
  ...
  const [activeTab, setActiveTab] = useState<ProfileTabKey>(
    showListings ? "listings" : "about"
  );
  const [aboutText, setAboutText] = useState<string | undefined>(undefined);

  const activeItem: ProfileNavKey = showListings ? "listings" : "profile";

  useEffect(() => {
    if (showListings) void loadMyListings();
  }, [showListings, loadMyListings]);
  ...
  const openTab = (tab: ProfileTabKey) => {
    if (tab === "listings") {
          ...
          <div className="mx-auto w-full max-w-[1600px] space-y-3 px-3 py-3 sm:space-y-4 sm:px-4 sm:py-4 lg:px-6">
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
              <div className="space-y-4">
                <ProfileTabs activeTab={showListings ? "listings" : activeTab} onChange={openTab} />
